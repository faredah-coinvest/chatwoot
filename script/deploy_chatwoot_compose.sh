...
if ! $COMPOSE -f "$COMPOSE_FILE" config >/dev/null; then
...
if [ "$SKIP_MIGRATE" = false ]; then
  ...
  $COMPOSE -f "$COMPOSE_FILE" run --rm --no-deps rails sh -lc \
...
# shellcheck disable=SC2086
$COMPOSE -f "$COMPOSE_FILE" up -d --no-deps $APP_SERVICES

echo "Current service status:"
$COMPOSE -f "$COMPOSE_FILE" ps rails sidekiq redis traefik || $COMPOSE -f "$COMPOSE_FILE" ps
